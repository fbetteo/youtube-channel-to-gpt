...
async function readConfig(): Promise<Json> {
  try {
    return JSON.parse(await readFile(CONFIG_PATH, "utf8")) as Json;
  } catch {
    return {};
  }
}

async function writeConfig(config: Json): Promise<void> {
...
async function getBaseUrl(): Promise<string> {
  ...
  return (
    process.env.YOUTUBE_TRANSCRIPT_API_BASE_URL ||
...
async function getApiKey(): Promise<string> {
  const config = await readConfig();
  const apiKey =
    process.env.YOUTUBE_TRANSCRIPT_API_KEY || (config.apiKey as string | undefined);
  if (!apiKey) {
    throw new Error(
      "Missing API key. Set YOUTUBE_TRANSCRIPT_API_KEY or run: ytx auth set-key <api_key>",
    );
  }
  return apiKey;
}
...
function optionsFromArgs(args: string[]): Json {
  return {
...
async function waitForJob(jobId: string): Promise<Json> {
  ...
  while (Date.now() - started < timeoutMs) {
    const status = (await requestJson(`/api/v1/jobs/${jobId}`)) as Json;
    console.error(
...
async function command(args: string[]): Promise<void> {
  ...
  if (area === "auth" && action === "set-key") {
    if (!value) throw new Error("Usage: ytx auth set-key <api_key>");
    const config = await readConfig();
    config.apiKey = value;
    const baseUrl = getFlag(args, "--base-url");
    if (baseUrl) config.apiBaseUrl = baseUrl.replace(/\/+$/, "");
  ...
  if (area === "channel" && action === "info") {
    ...
    printPayload(
      await requestJson(`/api/v1/channels/${encodeURIComponent(channel)}/info`),
      hasFlag(args, "--json"),
    );
    return;
  }
  ...
  for await (const line of rl) {
    ...
    try {
      ...
      if (method === "initialize") {
        ...
        const payload = await callTool(String(params.name), (params.arguments || {}) as Json);
        result = {
